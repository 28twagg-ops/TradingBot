...
_BOT_ROOT = ROOT.parent.parent / "TradingBot-git"
if not _BOT_ROOT.exists():
    _BOT_ROOT = ROOT.parent
OUT_DIR = ROOT / "results" / "pdt_removal"
...
# Live stop overshoot vs -0.5% model (Jun 2026 profile); used when samples unavailable
...
class SimMode:
    ...
    broker_stop_whole_share: bool = False
    single_window_per_day: bool = False  # any_time: one entry window per day
...
# Phase 3 D3 schedule modes (+ dual_window addendum)
D3_SCHEDULE_MODES: List[SimMode] = [
    SimMode("evening_only", "Entries at afternoon scan only",
            buy_morning=False, buy_afternoon=True,
            sell_morning=True, sell_intraday=True, sell_afternoon_eod=True,
            same_day_sell=True),
    SimMode("morning_only", "Entries at morning scan only",
            buy_morning=True, buy_afternoon=False,
            sell_morning=True, sell_intraday=True, sell_afternoon_eod=True,
            same_day_sell=True),
    SimMode("any_time", "One window per day — morning if signals, else evening",
            buy_morning=True, buy_afternoon=True,
            sell_morning=True, sell_intraday=True, sell_afternoon_eod=True,
            same_day_sell=True, single_window_per_day=True),
    SimMode("overnight_pdt", "Afternoon buy / morning sell (legacy overnight)",
            buy_morning=False, buy_afternoon=True,
            sell_morning=True, sell_intraday=False, sell_afternoon_eod=False,
            same_day_sell=False),
    SimMode("dual_window", "Both morning and evening entry scans same day",
            buy_morning=True, buy_afternoon=True,
            sell_morning=True, sell_intraday=True, sell_afternoon_eod=True,
            same_day_sell=True),
]
...
def _load_empirical_stop_samples_pct() -> List[float]:
    """Live stop-loss SELL pnl_pct values (percent units, e.g. -1.68)."""
    candidates = [
        ROOT.parent.parent / "TradingBot-git" / "logs" / "transactions.csv",
        ROOT.parent / "TradingBot-git" / "logs" / "transactions.csv",
        ROOT.parent / "logs" / "transactions.csv",
    ]
    for p in candidates:
        if not p.exists():
            continue
            ...
            with open(p, newline="", encoding="utf-8") as f:
                for r in csv.DictReader(f):
                    if (r.get("action") or "").strip().upper() != "SELL":
                        continue
                    if "stop_loss" not in (r.get("exit_reason") or "").lower():
                        continue
                    vals.append(float(r.get("pnl_pct") or 0))
            if len(vals) >= 20:
                return vals
        except Exception:
            continue
    # Fallback: model stop + mean live overshoot
    return [EXIT_STOP_LOSS * 100.0 + EMPIRICAL_STOP_OVERSHOOT_PP]
...
def _evaluate_exits(
    ...
    for ticker, pos in list(positions.items()):
        ...
        if exit_price is not None:
            cash = _try_exit(
                pos, day_idx, dates, df, exit_price, reason, cash, trades, mode,
                empirical_stops=empirical_stops,
                stop_samples_pct=stop_samples_pct,
                stop_rng=stop_rng,
            )
...
def run_simulation(
    ...
    stop_samples_pct = _load_empirical_stop_samples_pct() if empirical_stops else None
    stop_rng = np.random.default_rng(empirical_seed) if empirical_stops else None
    cash = starting_equity
    positions: Dict[str, Position] = {}
    trades: List[Trade] = []
    day_trades = 0
    equity_curve: List[float] = []
    daily_windows: Dict[str, set] = {}
    ...
    for day_idx in range(start_idx, end_idx + 1):
        ...
        if mode.buy_morning and day_idx > start_idx:
            ...
            for sig in signals:
                if entries_today >= n_slots:
                    break
                if mode.max_open_positions and len(positions) >= mode.max_open_positions:
                    break
                ticker = sig["ticker"]
                ...
                entries_today += 1
                morning_entries += 1
                daily_windows.setdefault(str(dt.date()), set()).add("morning")
        ...
        if mode.buy_afternoon and not (mode.single_window_per_day and morning_entries > 0):
            ...
            for sig in signals:
                ...
                cash -= da
                entries_today += 1
                daily_windows.setdefault(str(dt.date()), set()).add("evening")
    ...
    dw_stats = {
        "days_both_windows": sum(1 for w in daily_windows.values() if "morning" in w and "evening" in w),
        "days_one_window": sum(1 for w in daily_windows.values() if len(w) == 1),
        "morning_entry_trades": sum(1 for t in trades if t.entry_scan_window == "morning"),
        "evening_entry_trades": sum(1 for t in trades if t.entry_scan_window == "evening"),
    }
